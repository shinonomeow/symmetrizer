...
from my_s.nn.cartpole_networks import (
    BasisCartpoleNetworkWrapper,
    BasisCartpoleLayer,
    SingleBasisCartpoleLayer,
)
from my_s.groups.ops import closed_group
...
def test_prebuilt_policy():
    ...
    p_m2 = representations[1].detach().cpu().numpy()  # -1*np.eye(4)

    t_x = torch.Tensor(x).unsqueeze(0)  # 1,4
    t_p_x1 = torch.Tensor(np.matmul(x, p_m2)).unsqueeze(0)
    # print(t_x)
    # print(t_p_x1)
    #
    # print(t_x.shape, t_p_x1.shape)
    # print(type(t_x), type(t_p_x1))
    out_x = network(t_x)
    out_p_x1 = network(t_p_x1)
    # print(out_x.shape, out_p_x1.shape)
    # print(out_x, out_p_x1)
    # print(type(out_x), type(out_p_x1))
    #
...
def test_single_layer():
    ...
    x, _ = env.reset()
    t_x = torch.Tensor(x).unsqueeze(0)
    # t_x = torch.tensor(x, dtype=torch.float32).unsqueeze(0)
