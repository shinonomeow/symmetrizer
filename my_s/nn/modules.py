...
from my_s.groups.groups import MatrixRepresentation
from my_s.ops.ops import get_basis, get_coeffs, get_invariant_basis, compute_gain
...
class BasisConvolutionalLayer(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        self.W = torch.reshape(
            self.W,
            [
                self.channels_out * self.repr_size_out,
                self.channels_in * self.repr_size_in,
                self.fx,
                self.fy,
            ],
        )
        out = F.conv2d(x, self.W, stride=self.stride, padding=self.padding, bias=None)
        if self.has_bias:
            self.b = torch.sum(self.basis_bias * self.coeffs_bias, 0)
            self.b = torch.reshape(
                self.b, [1, self.channels_out * self.repr_size_out, 1, 1]
            )
            return out + self.b
        else:
            return out
...
class BasisLinear(BasisLayer):
    ...
    def __init__(
        ...
        self.group: MatrixRepresentation = group
        self.space: str = basis
        self.repr_size_in: int = group.repr_size_in
        self.repr_size_out: int = group.repr_size_out
        ...
        if bias:
            ...
            basis_bias, self.rank_bias = get_invariant_basis(
                size, group, new_size, space=self.space
            )
